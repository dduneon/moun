...
def get_billing_summary(db: Session, cycle_id: int) -> BillingSummary:
    """billing_cycle 기준: 청구일이 이 사이클에 속하는 소비 합계 (카테고리별)."""
    rows = db.execute(
        select(Category.id, Category.name, func.sum(Transaction.amount))
        .join(Category, Transaction.category_id == Category.id)
        .where(Transaction.billing_cycle_id == cycle_id)
        .group_by(Category.id, Category.name)
    ).all()
...
def get_available_budget(db: Session, cycle_id: int) -> AvailableBudget:
    """
    가용 예산 = salary + extra_income - fixed_expense - billed_transactions

    salary: cycle.salary_actual 우선, 없으면 salary_expected
    extra_income: 해당 사이클 extra income 합계
    fixed_expense: billing_day가 사이클 기간 내에 있는 active 고정지출 합계
    billed_transactions: billing_cycle_id == cycle_id 인 transaction 합계
    """
    ...
    extra_row = db.scalar(
        ...
        .where(Income.budget_cycle_id == cycle_id, Income.type == IncomeType.extra)
    )
    extra_income = Decimal(str(extra_row))

    # billing_day가 start_date.day ~ end_date.day 범위 안에 있는 고정지출
    # 사이클은 항상 21일 시작 ~ 익월 20일 종료이므로, billing_day 1~20은 end_date 월,
    # billing_day 21~31은 start_date 월에 해당 → 모든 billing_day를 포함
    fixed_row = db.scalar(
    ...
    billing_summary = get_billing_summary(db, cycle_id)
    ...
    return AvailableBudget(
        ...
        available=available,
        spend_summary=get_spend_summary(db, cycle_id),
